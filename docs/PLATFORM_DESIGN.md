...
Platform 是面向**软件工程师、技术管理者、以及非软件专业人员**的 Web 分析监控平台。它是 Agent 活动的"事后分析"工具，同时承担**非程序员友好**的职责——让不熟悉终端的用户也能理解和追踪 Agent 的工作过程。与 TUI（"开发者工作台"）互补，共同组成完整的 Agent Coding 系统。

| 维度 | TUI | Platform |
|------|-----|----------|
| 用户 | 开发人员（FDE、软件工程师） | 软件工程师、技术管理者、**非软件专业人员** |
| 时机 | 实时交互 | 事后分析 / 持续监控 |
| 形态 | 终端界面 | Web 应用 |
| 模式 | "做"——指挥 Agent 工作 | "看"——理解 Agent 行为 |
| 设计原则 | 开发人员优先，不为非技术用户妥协 | **非程序员友好**，可视化、直观、低门槛 |
