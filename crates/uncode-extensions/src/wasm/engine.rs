...
fn parse_lifecycle_hook(name: &str) -> Option<LifecycleHook> {
    match name {
        ...
        "session_shutdown" => Some(LifecycleHook::SessionShutdown),
        "session_before_compact" => Some(LifecycleHook::SessionBeforeCompact),
        "session_compact" => Some(LifecycleHook::SessionCompact),
        "before_agent_start" => Some(LifecycleHook::BeforeAgentStart),
        "agent_start" => Some(LifecycleHook::AgentStart),
        "agent_end" => Some(LifecycleHook::AgentEnd),
        "context" => Some(LifecycleHook::Context),
        "before_provider_request" => Some(LifecycleHook::BeforeProviderRequest),
        "after_provider_response" => Some(LifecycleHook::AfterProviderResponse),
        "message_update" => Some(LifecycleHook::MessageUpdate),
        "message_start" => Some(LifecycleHook::MessageStart),
        "message_end" => Some(LifecycleHook::MessageEnd),
        "model_select" => Some(LifecycleHook::ModelSelect),
        "tool_execution_start" => Some(LifecycleHook::ToolExecutionStart),
        "tool_execution_update" => Some(LifecycleHook::ToolExecutionUpdate),
        "tool_execution_end" => Some(LifecycleHook::ToolExecutionEnd),
        "resources_discover" => Some(LifecycleHook::ResourcesDiscover),
        "session_before_switch" => Some(LifecycleHook::SessionBeforeSwitch),
        "session_before_fork" => Some(LifecycleHook::SessionBeforeFork),
        "session_before_tree" => Some(LifecycleHook::SessionBeforeTree),
        "session_tree" => Some(LifecycleHook::SessionTree),
        "input" => Some(LifecycleHook::Input),
        "thinking_level_select" => Some(LifecycleHook::ThinkingLevelSelect),
        "user_bash" => Some(LifecycleHook::UserBash),
        _ => {
